...
public class Track implements ListADT<Pod>{
  ...
  public boolean addPassenger(String name, boolean isFirstClass) {
    ...
    while(current != null) {
      try {
        ...
        if (pod.isFunctional() && ((isFirstClass && pod.getPodClass() == Pod.FIRST) || 
            (!isFirstClass && pod.getPodClass() == Pod.ECONOMY))) {
   ...
   * @return  the index of the Pod this passenger was located in, or -1 if they were not found 
   * (or the Track is currently empty)
   */
  public int findPassenger(String name) {
    ...
    while (current != null) {
      Pod pod = current.getPod();
      try {
        if (pod.isFunctional() && pod.containsPassenger(name)) {
          return index;
        }
      }
      catch (MalfunctioningPodException e) {
  ...
  /**
   * Reports the current number of Pods currently on this track. 
   * This number includes both functional and non-functional Pods.
   * 
   ...
   * exception to be thrown. Attempting to add a non-functional Pod should simply not cause 
   * the list to change.
   * 
